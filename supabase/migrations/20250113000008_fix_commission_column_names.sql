-- Fix commission trigger to use correct column names from users table
-- Users table has: sales_commission_rate, marketing_commission_rate, production_commission_rate
-- NOT: commission_rate, sales_manager_commission_rate, production_manager_commission_rate, paid_when
...
BEGIN
  ...
  IF v_lead.sales_rep_id IS NOT NULL THEN
    IF NOT EXISTS (
      ...
      IF v_user IS NOT NULL THEN
        -- FIXED: users.sales_commission_rate (not commission_rate)
        v_commission_amount := v_base_amount * (COALESCE(v_user.sales_commission_rate, 10) / 100);
        
        IF v_commission_amount > 0 OR COALESCE(v_user.sales_commission_rate, 10) > 0 THEN
          INSERT INTO lead_commissions (
            ...
            'percentage', COALESCE(v_user.sales_commission_rate, 10), NULL,
            v_commission_amount, v_base_amount, 
            'when_final_payment', 'pending',
  ...
  -- 2. MARKETING REP COMMISSION (already correct)
  ...
  -- 3. SALES MANAGER COMMISSION (FIXED: use sales_commission_rate for managers too)
  IF v_lead.sales_manager_id IS NOT NULL THEN
    IF NOT EXISTS (
      ...
      IF v_sales_manager IS NOT NULL AND v_sales_manager.sales_commission_rate IS NOT NULL THEN
        v_commission_amount := v_base_amount * (v_sales_manager.sales_commission_rate / 100);
        
        IF v_commission_amount > 0 OR v_sales_manager.sales_commission_rate > 0 THEN
          INSERT INTO lead_commissions (
            company_id, lead_id, user_id, assignment_field,
            commission_type, commission_rate, flat_amount,
            calculated_amount, base_amount, paid_when, status,
            ...
            'percentage', v_sales_manager.sales_commission_rate, NULL,
            v_commission_amount, v_base_amount, 
            'when_final_payment', 'pending',
  ...
  -- 4. PRODUCTION MANAGER COMMISSION (FIXED: use production_commission_rate)
  IF v_lead.production_manager_id IS NOT NULL THEN
    IF NOT EXISTS (
      ...
      IF v_production_manager IS NOT NULL AND v_production_manager.production_commission_rate IS NOT NULL THEN
        v_commission_amount := v_base_amount * (v_production_manager.production_commission_rate / 100);
        
        IF v_commission_amount > 0 OR v_production_manager.production_commission_rate > 0 THEN
          INSERT INTO lead_commissions (
            company_id, lead_id, user_id, assignment_field,
            commission_type, commission_rate, flat_amount,
            calculated_amount, base_amount, paid_when, status,
            ...
            'percentage', v_production_manager.production_commission_rate, NULL,
            v_commission_amount, v_base_amount, 
            'when_final_payment', 'pending',
