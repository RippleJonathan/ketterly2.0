-- =====================================================================
-- FIX TEAM LEAD COMMISSION IN AUTO-CREATE TRIGGER
-- Ensures team lead commissions are created when invoice is created
-- =====================================================================
...
BEGIN
  ...
  IF v_lead.sales_rep_id IS NOT NULL AND v_lead.location_id IS NOT NULL THEN
    -- Find sales rep's team via location_users
    SELECT 
      lu.*,
      t.team_lead_id,
      t.commission_rate as team_commission_rate,
      t.paid_when as team_paid_when,
      t.is_active as team_is_active
    INTO v_sales_rep_team
    FROM location_users lu
    ...
    IF v_sales_rep_team IS NOT NULL THEN
      RAISE NOTICE 'Sales rep is on team with Team Lead: %', v_sales_rep_team.team_lead_id;
      
      -- Check if team lead commission already exists
      IF NOT EXISTS (
        SELECT 1 FROM lead_commissions
        WHERE lead_id = NEW.lead_id
        AND user_id = v_sales_rep_team.team_lead_id
        AND assignment_field = 'team_lead_override'
        AND deleted_at IS NULL
      ) THEN
        -- Calculate team lead commission
        v_commission_rate := COALESCE(v_sales_rep_team.team_commission_rate, 0);
        v_commission_amount := v_base_amount * (v_commission_rate / 100);
        ...
        IF v_commission_amount > 0 OR v_commission_rate > 0 THEN
          INSERT INTO lead_commissions (
            ...
            NEW.company_id, NEW.lead_id, v_sales_rep_team.team_lead_id, 'team_lead_override',
            'percentage', v_commission_rate, NULL,
            v_commission_amount, v_base_amount, 
            COALESCE(v_sales_rep_team.team_paid_when, 'when_final_payment'), 'pending',
...
COMMENT ON FUNCTION auto_create_commissions_on_invoice IS 'Auto-creates commissions for assigned users when invoice is created from contract (updated to fix team lead logic)';
