// Estimate Template Dialog Component
// Mirror of material-template-dialog.tsx for estimate templates
...
import {
  ...
  useBulkAddMaterialsToEstimateTemplate,
  useRemoveMaterialFromEstimateTemplate,
  useTemplateEstimateItems,
...
interface EstimateTemplateDialogProps {
  isOpen: boolean
  onClose: () => void
  template?: EstimateTemplate | null
}

export function EstimateTemplateDialog({ isOpen, onClose, template }: EstimateTemplateDialogProps) {
  const { data: company } = useCurrentCompany()
  const createTemplate = useCreateEstimateTemplate()
  const updateTemplate = useUpdateEstimateTemplate()
  const bulkAddMaterials = useBulkAddMaterialsToEstimateTemplate()
  const removeMaterial = useRemoveMaterialFromEstimateTemplate()
  ...
  const { data: templateItemsData } = useTemplateEstimateItems(template?.id)
  ...
  const onSubmit = async (data: TemplateFormData) => {
    ...
    if (isEditing && template) {
      ...
      if (selectedMaterials.length > 0) {
        // Get per_square from material defaults
        const materialsWithDefaults = selectedMaterials.map(sm => ({
          material_id: sm.material.id,
          per_square: sm.material.default_per_square || sm.material.default_per_unit || 1,
  ...
  const addMaterialToTemplate = async () => {
    ...
    await bulkAddMaterials.mutateAsync({
      ...
      materials: [{
        ...
        per_square: material.default_per_square || material.default_per_unit || 1,
        description: material.name,
      }],
    })
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>
            {isEditing ? 'Edit Estimate Template' : 'Create Estimate Template'}
          </DialogTitle>
        </DialogHeader>
          ...
          <div className="space-y-4">
            <div className="grid gap-4 md:grid-cols-2">
              <div className="space-y-2">
                ...
                <Input
                  id="name"
                  placeholder="Standard Residential Roof Estimate"
                  {...register('name')}
                />
                {errors.name && (
            ...
            <div className="space-y-2">
              ...
              <Textarea
                id="description"
                placeholder="Optional description of this estimate template..."
                rows={2}
          ...
          {!isEditing && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="font-semibold">Template Line Items</h3>
                  <p className="text-sm text-muted-foreground">
                    Add materials from your catalog - quantities will auto-calculate from measurements
                  </p>
                </div>
              </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="font-semibold">Template Line Items</h3>
                  <p className="text-sm text-muted-foreground">
                    Add materials from your catalog to this estimate template
                  </p>
                </div>
              </div>
              ...
              <div className="flex gap-2">
                <div className="flex-1">
                  <Select value={selectedMaterialId} onValueChange={setSelectedMaterialId}>
                    ...
                    <SelectContent>
                      {materials
                        .filter(m => !templateItems.some(tm => tm.material_id === m.id))
                        .map(material => (
                          <SelectItem key={material.id} value={material.id}>
                            <div className="flex items-center gap-2">
                              <Package className="h-4 w-4" />
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          <Badge variant="secondary" className="text-xs">
                            {item.material?.measurement_type === 'square' ? 'Per Square' :
                             item.material?.measurement_type === 'hip_ridge' ? 'Hip+Ridge' :
                             item.material?.measurement_type === 'perimeter' ? 'Perimeter' :
                             item.material?.measurement_type || 'Per Square'}
                          </Badge>
                        </div>
                        <p className="text-sm text-muted-foreground">
                          {item.per_square} {item.material?.unit}
                          {item.material?.measurement_type === 'square' ? ' per square' :
                           item.material?.measurement_type === 'each' ? ' (fixed)' :
                           ' per LF'}
                          {item.description && item.description !== item.material?.name && ` • ${item.description}`}
                        </p>
                      </div>
                      <Button
                        type="button"
