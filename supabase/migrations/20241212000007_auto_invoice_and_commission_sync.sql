-- Auto-create invoices from quotes and update from change orders
-- This makes invoices the single source of truth for financials and commissions

-- =============================================================================
-- FUNCTION: Auto-create invoice when quote is accepted
-- =============================================================================
...
-- Trigger to auto-create invoice when quote is accepted
DROP TRIGGER IF EXISTS trigger_auto_create_invoice_from_quote ON quotes;
CREATE TRIGGER trigger_auto_create_invoice_from_quote
  AFTER UPDATE OF status ON quotes
  FOR EACH ROW
  EXECUTE FUNCTION auto_create_invoice_from_quote();

COMMENT ON FUNCTION auto_create_invoice_from_quote IS 'Automatically create invoice with line items when quote is accepted/approved';

-- =============================================================================
-- FUNCTION: Auto-update invoice when change order is approved
-- =============================================================================
...
BEGIN
  ...
  IF NEW.status = 'approved' AND 
    ...
    -- Recalculate invoice totals (handled by trigger below)
...
-- Trigger to auto-update invoice when change order is approved
DROP TRIGGER IF EXISTS trigger_auto_update_invoice_from_change_order ON change_orders;
CREATE TRIGGER trigger_auto_update_invoice_from_change_order
  AFTER UPDATE OF status ON change_orders
  FOR EACH ROW
  EXECUTE FUNCTION auto_update_invoice_from_change_order();

COMMENT ON FUNCTION auto_update_invoice_from_change_order IS 'Automatically add change order to invoice when approved';

-- =============================================================================
-- FUNCTION: Recalculate invoice totals when line items change
-- =============================================================================
...
COMMENT ON FUNCTION recalculate_invoice_totals IS 'Automatically recalculate invoice totals when line items are added/updated/deleted';

-- =============================================================================
-- FUNCTION: Update commission when invoice total changes
-- =============================================================================
...
-- Trigger to update commissions when invoice total changes
DROP TRIGGER IF EXISTS trigger_auto_update_commission_from_invoice ON customer_invoices;
CREATE TRIGGER trigger_auto_update_commission_from_invoice
  AFTER UPDATE OF total ON customer_invoices
  FOR EACH ROW
  EXECUTE FUNCTION auto_update_commission_from_invoice();

COMMENT ON FUNCTION auto_update_commission_from_invoice IS 'Automatically update commission amounts when invoice total changes';
