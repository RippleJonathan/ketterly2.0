...
export function MultiUserAssignments({ 
  ...
  const renderDropdown = (label: string, field: string, currentValue: string | null) => (
  ...
  return (
    <div className="space-y-4">
      {renderDropdown('Sales Rep', 'sales_rep_id', salesRepId)}
      {renderDropdown('Marketing Rep', 'marketing_rep_id', marketingRepId)}
      {renderDropdown('Sales Manager', 'sales_manager_id', salesManagerId)}
      {renderDropdown('Production Manager', 'production_manager_id', productionManagerId)}
    </div>
  )
}
